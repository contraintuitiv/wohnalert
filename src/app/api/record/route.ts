...
function parseDegewo(data: string[], extractedRecords: ExtractedRecord[]) {
  ...
  data
    .filter((line) => !line.endsWith("Merken") && !line.startsWith(" /"))
    .forEach((line) => {
      if (line.startsWith("    ")) {
        if (line.includes("|")) {
          line.trim().replace("|", ", ");
      ...
      if (line.startsWith("      ")) {
        ...
        line.split(/(?<=Zimmer)|(?<=m²)| – /).forEach((part) => {
          ...
          if (!part.includes("Zimmer") && !part.includes("m²")) {
            ...
            if (
              part.includes("mit WBS" || "WBS erforderlich" || "WBS erwünscht")
            ) {
              setProperty("wbs", "true", extractedRecords);
              return;
            }
...
export async function POST(req: NextRequest) {
  ...
  if (!validator.isUUID(uuid))
    return new NextResponse("invalid body, must be valid UUID in plain text", {
      status: 400,
    });
  ...
  for (const record of extractedRecords) {
    try {
        ...
        where: {
          AND: [
            ...
            },
            {
              OR: [
                { boroughs: null },
                { boroughs: { contains: createdRecord.borough } },
              ],
            },
            {
              OR: [
                { landlords: null },
                { landlords: { contains: createdRecord.landlord } },
              ],
            },
          ],
        },
      ...
      for (const ntfy of ntfys) {
        ...
        Sentry.captureMessage(
          `Sent ${createdRecord.id}/${createdRecord.description} to https://${
            ntfy.host || process.env.NTFY_HOST
          }/${ntfy.topic || ntfy.id}`
        );
      }
    }
